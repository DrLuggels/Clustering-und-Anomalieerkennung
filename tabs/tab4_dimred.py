...
from utils.explanations import PCA_INTRO, UMAP_INTRO, TSNE_INTRO
...
def render():
    ...
    if n_features < 3:
        st.info(
            ...
            "aber du kannst trotzdem sehen, wie PCA und die Reduktion die Daten transformieren."
        )

    # Detect whether UMAP is available
    try:
        import umap as _umap_test
        _has_umap = True
    except ImportError:
        _has_umap = False

    reduction_method = "UMAP" if _has_umap else "t-SNE"
    ...
    with col_ctrl2:
        if _has_umap:
            ...
            st.markdown(TSNE_INTRO)
            tsne_perplexity = st.slider("t-SNE: perplexity", 5, 50, 30, key="tsne_perp")
            tsne_learning_rate = st.slider("t-SNE: learning_rate", 10.0, 1000.0, 200.0,
                                           step=10.0, key="tsne_lr")
    ...
    if _has_umap:
        ...
        df_red = pd.DataFrame(X_red, columns=["UMAP1", "UMAP2"])
        red_cols = ["UMAP1", "UMAP2"]
    else:
        from sklearn.manifold import TSNE
        perp = min(tsne_perplexity, max(1, len(X) - 1))
        reducer = TSNE(n_components=2, perplexity=perp,
                       learning_rate=tsne_learning_rate, random_state=42)
        X_red = reducer.fit_transform(X)
        df_red = pd.DataFrame(X_red, columns=["t-SNE1", "t-SNE2"])
        red_cols = ["t-SNE1", "t-SNE2"]

    # --- Side by Side ---
    st.subheader(f"PCA vs. {reduction_method}")
    col_pca, col_red = st.columns(2)
    ...
    trust_red = trustworthiness(X, X_red, n_neighbors=5)
    col_t2.metric(f"Trustworthiness {reduction_method}", f"{trust_red:.4f}")
    ...
    st.session_state["data_umap"] = df_red
